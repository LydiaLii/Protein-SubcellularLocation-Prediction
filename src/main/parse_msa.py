...
for category in cata_list:
    ...
    s = re.findall(category+'_(\d{3,4})_hello', content)
    num_list = [int(x) for x in set(s)]
    seq_num = sorted(num_list, reverse=True)[0]

    s = re.findall(category+'_1_hello(.*?)\d{2,4}', content)
    aligned_seq = ''.join([x.strip() for x in s])
    ...
    for i in range(seq_num):  # seq_num
        bar.update()
        s = re.findall(category+'_'+str(i+1)+'_hello(.*?)\d{2,4}', content)
        aligned_seq = ''.join([x.strip() for x in s])
