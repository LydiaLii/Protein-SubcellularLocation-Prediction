...
W4 = tf.Variable(tf.truncated_normal([L, P], stddev=0.1))
...
update_ema = tf.group(update_ema1, update_ema4)
...
with tf.Session() as sess:
    ...
    for iteration in range(train_iteration):
        ...
        for i in range(total_batch):
            ...
            o, c, accu, summary_str = sess.run([optimizer, cost_function, accuracy, merge_summary_op],
                                               feed_dict={x: batch_xs, y: batch_ys, iter: i, tst: False, pkeep: keep_rate})
    ...
    accu = accuracy.eval({x: test_images, y: test_labels, tst: True, pkeep: keep_rate})
